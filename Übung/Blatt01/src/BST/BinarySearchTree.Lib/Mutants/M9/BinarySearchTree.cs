namespace BinarySearchTree.Lib.Mutants.M9;
...
public class BinarySearchTree
{
    ...
    /// <summary>
    /// Mutation 9: Return the count of the root node only.
    /// </summary>
    ...
    /// <summary>
    /// Helper method to recursively calculate the sum of all values.
    /// </summary>
    private static int SumValues(Node? node)
    {
        if (node is null) return 0;

        return node.Value * node.Count + SumValues(node.Left) + SumValues(node.Right);
    }
